...
public class DaosUsuario {
    ...
    public List<UsuarioE> fillList(Connection con, String variable, String valor, boolean exactamente, boolean imagen) {
    ...
    public String crearUsuario(UsuarioE ue, List<UsuarioE> head, Historial cola) {
        boolean unico = true;
        for (int i = 0; i < head.size(); i++) {
            UsuarioE ce = head.get(i);
            if (ue.getTipo().equals("cliente")) {
                if (ue.getClienteCedula().equals(ce.getClienteCedula()) || ue.getClienteCedula().equals(ce.getEmpleadoCedula())) {
                    unico = false;
                    break;
                }
            } else {
                if (!ue.getTipo().equals("cliente")) {
                    if (ue.getEmpleadoCedula().equals(ce.getEmpleadoCedula()) || ue.getEmpleadoCedula().equals(ce.getClienteCedula())) {
                        unico = false;
                        break;
                    }
                }
        ...
        if (unico) {
            UsuarioE ce = new UsuarioE(ue.getIdUsuario(), ue.getNombre(), ue.getTipo(), ue.getClienteCedula(), ue.getEmpleadoCedula(), ue.getCodigo(), ue.getApellido(), ue.getImagen());
            head.add(ce);
    ...
    public List<UsuarioE> buscarUsuario(List<UsuarioE> lista, String valor) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            if (stringIniciaCon(ce.getApellido(), valor)
                ...
                UsuarioE encontrado = new UsuarioE(ce.getIdUsuario(), ce.getNombre(), ce.getTipo(), ce.getClienteCedula(), ce.getEmpleadoCedula(), ce.getCodigo(), ce.getApellido(), ce.getImagen());
    ...
    public String actualizarUsuario(UsuarioE usuario, List<UsuarioE> lista, Historial registro) {
        UsuarioE reemplazo = new UsuarioE(usuario.getIdUsuario(), usuario.getNombre(), usuario.getTipo(), usuario.getClienteCedula(), usuario.getEmpleadoCedula(), usuario.getCodigo(), usuario.getApellido(), usuario.getImagen());
        for (int i = 0; i < lista.size(); i++) {
            UsuarioE ue = lista.get(i);
            if ((reemplazo.getTipo().equals("cliente") && reemplazo.getClienteCedula().equals(ue.getClienteCedula())) || (!reemplazo.getTipo().equals("cliente") && ue.getEmpleadoCedula().equals(reemplazo.getEmpleadoCedula()))) {
    ...
    public String eliminarUsuario(String id, List<UsuarioE> lista, Historial registro) {
        for (int i = 0; i < lista.size(); i++) {
            UsuarioE ce = lista.get(i);
                if (ce.getIdUsuario().equals(id)) {
                    lista.remove(i);
                    UsuarioE eliminado = new UsuarioE(ce.getIdUsuario(), ce.getNombre(), ce.getTipo(), ce.getClienteCedula(), ce.getEmpleadoCedula(), ce.getCodigo(), ce.getApellido(), ce.getImagen());
    ...
    /**
     * Busca coincidencias "al principio" de una palabra original, respecto aun
     * posible fragmento de esta
     *
     * @param original Es el String que usaremos como base o sobre el cual
     * buscaremos coincidencias
     * @param comparar Es el String que buscaremos en el String original
     * @return retorna un boolean, indicando si hay coincidencias o no
     */
